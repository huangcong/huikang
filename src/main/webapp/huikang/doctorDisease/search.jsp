    ...
    <tr>
      <td><div class="op_area">
          <table cellspacing="2" cellpadding="0" border="0" width="100%">
            ...
            <tr>
              <td class="func"><a onclick="show('${CTX}/huikang/doctorDisease/do.jsp?method=${MD_F}','<%=DoctorDisease.TABLE_ALIAS%>添加',600)" href="javascript:;" class="sysiconBtn addorder">添加</a></td>
            </tr>
          </table>
        </div></td>
      <td><div class="op_area">
          <table cellspacing="0" cellpadding="0" border="0" width="100%">
            ...
            <tr>
              <td class="func"><span  onclick="doBatchDelete('${ctx}/huikang/doctorDisease/do.jsp','items',document.forms.huikang_doctorDisease_page_form)" class="sysiconBtn delete">删除</span><a  onclick="showConfirm('/pages/admin/huikang/doctorDisease/batch_edit.jsp','批量编辑',500,300,function(){alert('编辑成功')},function(){alert('取消编辑')})"  class="sysiconBtn edit">批量编辑</a></td>
            </tr>
          </table>
        </div></td>
      <td><div class="op_area">
            ...
            <tr>
              <td class="func">
                  ...
                  <div id="search_button_rel" style="display:none; position:absolute;" class="finder_filter_pos">
                    ...
                    <!--  <table>
						<tr>	
							<td class="tdLabel"><%=DoctorDisease.ALIAS_DOCTOR_ID%>：</td>		
							<td>
								<input value="${query.doctorId}" id="doctorId" name="doctorId" maxlength="19"  class="digits "/>
							</td>
							<td class="tdLabel"><%=DoctorDisease.ALIAS_DISEASE_ID%>：</td>		
							<td>
				  ...
				  <table>
						<tr>	
							...
							<select id="searchColumn" name="searchColumn" onchange="syncValue(this,'#searchColumnSel')">
								 <option value="doctorId" <c:if test="${'doctorId' eq searchColumn}"> selected</c:if>><%=DoctorDisease.ALIAS_DOCTOR_ID%></option>
								 <option value="diseaseId" <c:if test="${'diseaseId' eq searchColumn}"> selected</c:if>><%=DoctorDisease.ALIAS_DISEASE_ID%></option>
                          </select>：</td>		
							<td>
                    ...
                    <table cellspacing="0" cellpadding="0" border="0" width="100%">
                      <tr>
                        <td><select id="searchColumnSel" name="searchColumnSel"  onchange="syncValue(this,'#searchColumn')">
								 <option <c:if test="${'doctorId' eq searchColumn}"> selected</c:if> value="doctorId"><%=DoctorDisease.ALIAS_DOCTOR_ID%></option>
								 <option <c:if test="${'diseaseId' eq searchColumn}"> selected</c:if> value="diseaseId"><%=DoctorDisease.ALIAS_DISEASE_ID%></option>
                          </select></td>
