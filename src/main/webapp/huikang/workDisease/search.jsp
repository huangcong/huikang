    ...
    <tr>
      <td><div class="op_area">
          <table cellspacing="2" cellpadding="0" border="0" width="100%">
            ...
            <tr>
              <td class="func"><a onclick="show('${CTX}/huikang/workDisease/do.jsp?method=${MD_F}','<%=WorkDisease.TABLE_ALIAS%>添加',600)" href="javascript:;" class="sysiconBtn addorder">添加</a></td>
            </tr>
          </table>
        </div></td>
      <td><div class="op_area">
          <table cellspacing="0" cellpadding="0" border="0" width="100%">
            ...
            <tr>
              <td class="func"><span  onclick="doBatchDelete('${ctx}/huikang/workDisease/do.jsp','items',document.forms.huikang_workDisease_page_form)" class="sysiconBtn delete">删除</span><a  onclick="showConfirm('/pages/admin/huikang/workDisease/batch_edit.jsp','批量编辑',500,300,function(){alert('编辑成功')},function(){alert('取消编辑')})"  class="sysiconBtn edit">批量编辑</a></td>
            </tr>
          </table>
        </div></td>
      <td><div class="op_area">
            ...
            <tr>
              <td class="func">
                  ...
                  <div id="search_button_rel" style="display:none; position:absolute;" class="finder_filter_pos">
                    ...
                    <!--  <table>
						<tr>	
							<td class="tdLabel"><%=WorkDisease.ALIAS_WORK_ID%>：</td>		
							<td>
								<input value="${query.workId}" id="workId" name="workId" maxlength="19"  class="digits "/>
							</td>
							<td class="tdLabel"><%=WorkDisease.ALIAS_DISEASE_ID%>：</td>		
							<td>
				  ...
				  <table>
						<tr>	
							...
							<select id="searchColumn" name="searchColumn" onchange="syncValue(this,'#searchColumnSel')">
								 <option value="workId" <c:if test="${'workId' eq searchColumn}"> selected</c:if>><%=WorkDisease.ALIAS_WORK_ID%></option>
								 <option value="diseaseId" <c:if test="${'diseaseId' eq searchColumn}"> selected</c:if>><%=WorkDisease.ALIAS_DISEASE_ID%></option>
                          </select>：</td>		
							<td>
                    ...
                    <table cellspacing="0" cellpadding="0" border="0" width="100%">
                      <tr>
                        <td><select id="searchColumnSel" name="searchColumnSel"  onchange="syncValue(this,'#searchColumn')">
								 <option <c:if test="${'workId' eq searchColumn}"> selected</c:if> value="workId"><%=WorkDisease.ALIAS_WORK_ID%></option>
								 <option <c:if test="${'diseaseId' eq searchColumn}"> selected</c:if> value="diseaseId"><%=WorkDisease.ALIAS_DISEASE_ID%></option>
                          </select></td>
